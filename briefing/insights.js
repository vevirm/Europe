(function(root,factory){
  ...
  const TOPICS=[
    {name:"Raw materials", terms:["critical raw material", "critical raw materials", "critical mineral", "critical minerals", "rare earth", "rare earths", "lithium", "cobalt", "nickel", "graphite", "copper", "gallium", "germanium", "tungsten", "magnesium", "magnet", "magnets", "mining", "refining", "processing capacity", "stockpile", "stockpiling", "resourceeu"]},
    {name:"Energy", terms:["energy security", "gas", "lng", "natural gas", "oil", "pipeline", "electricity", "power grid", "electricity grid", "energy imports", "energy prices", "nuclear fuel", "uranium", "repowereu"]},
    {name:"Supply chains & dependencies", terms:["supply chain", "supply chains", "value chain", "strategic dependency", "strategic dependencies", "critical dependency", "import dependence", "single supplier", "diversification", "diversify", "reshoring", "friendshoring", "nearshoring", "shortage", "shortages", "bottleneck"]},
    {name:"Technology security", terms:["export control", "export controls", "dual-use", "dual use", "technology security", "technology leakage", "entity list", "outbound investment", "semiconductor", "semiconductors", "chips", "quantum", "artificial intelligence", " ai ", "critical technology", "critical technologies"]},
    {name:"Investment & ownership", terms:["foreign direct investment", "fdi", "investment screening", "fdi screening", "foreign acquisition", "takeover", "acquisition", "stake", "foreign subsidies", "state-owned", "sovereign wealth", "greenfield"]},
    {name:"Trade policy", terms:["tariff", "tariffs", "trade defence", "trade defense", "anti-dumping", "anti-subsidy", "countervailing", "safeguard", "overcapacity", "market access", "wto", "trade war", "customs", "exports", "imports"]},
    {name:"Coercion & sanctions", terms:["economic coercion", "coercion", "anti-coercion", "weaponisation", "weaponization", "weaponised", "weaponized", "retaliation", "countermeasures", "sanction", "sanctions", "circumvention", "asset freeze", "price cap"]},
    {name:"Industry & competitiveness", terms:["industrial policy", "industrial capacity", "competitiveness", "manufacturing", "factory", "plant", "steel", "automotive", "chemicals", "batteries", "electric vehicles", "solar", "industrial accelerator act", "made in europe", "european preference", "deindustrialisation", "deindustrialization"]},
    {name:"Critical infrastructure", terms:["critical infrastructure", "port", "ports", "subsea cable", "undersea cable", "submarine cable", "telecom", "telecommunications", "5g", "high-risk vendor", "cyber", "cybersecurity", "sabotage", "data centre", "data center", "cloud"]},
    {name:"Defence industry", terms:["defence industry", "defense industry", "ammunition", "defence production", "rearm europe", "readiness 2030", "edip", "military", "nato"]},
    {name:"Finance & the euro", terms:["financial sanctions", "payment system", "payments", "swift", "digital euro", "international role of the euro", "capital markets", "savings and investments union", "dollar"]},
    {name:"Partners & agreements", terms:["trade agreement", "free trade agreement", "partnership", "partnerships", "like-minded", "g7", "global gateway", "mercosur", "india", "japan", "indo-pacific", "canada", "australia", "united kingdom", "african", "africa", "latin america"]},
    {name:"China & the US", terms:["china", "chinese", "united states", "u.s.", "us-china", "transatlantic", "washington", "beijing", "trump"]},
    {name:"Economic security strategy", terms:["economic security", "strategic autonomy", "open strategic autonomy", "de-risk", "derisk", "toolbox", "risk assessment", "geoeconomic", "economic statecraft"]}
  ];
  const OTHER='Other economic security';
  ...
  function fastReaderText(v){
    ...
    const replacements=[
      ...
      [/\bfragmented AI rules reduce Europe's ability to turn regulation into coherent market and technology leverage\b/gi,"AI rules split across countries reduce Europe's ability to create a consistent market and influence technology"],
      [/\bchip access and production determine whether European industry can use critical hardware without external restrictions\b/gi,'chip access and production determine whether Europe’s economic security can use essential hardware without outside restrictions'],
  ...
  function fallbackPoint(x){
    ...
    if(b){
      ...
      if(/scenario/.test(text)) return 'Scenario methods can test how uncertain strategic futures may unfold for Europe’s economy.';
      if(/backcasting/.test(text)) return 'Backcasting can connect long-term economic-security goals with decisions made today.';
      return 'Foresight methods can test emerging strategic change around Europe’s economic security.';
    }
    ...
    if(/dual.?use|military|defen[cs]e/.test(text)&&/innovation|technology|research/.test(text)) return 'Security competition is pulling more European industry toward defence and dual-use priorities.';
  ...
  function whyFor(x){
    ...
    const mechanism=(()=>{
      ...
      if(String(x?.strand||'').toUpperCase()==='B'||/foresight|horizon scan|weak signal|scenario planning|backcasting/.test(primary)){
        ...
        if(/scenario/.test(primary)) return 'Scenario methods matter because they let decision-makers test economic-security choices against several plausible strategic futures.';
      ...
      if(/high performance computing|hpc/.test(primary)) return 'Shared computing centres give European firms wider access to supercomputers, training and specialist support.';
      if(/research.?security|knowledge.?security|foreign interference/.test(primary)) return 'Technology-security rules change which firms Europe trades with and what know-how can cross borders.';
      if(/artificial intelligence| ai |ai-|ai4s|machine learning/.test(` ${all} `)) return 'AI policy, capital and infrastructure affect whether European firms can build and govern frontier capability.';
      if(/patent|intellectual property|\bip\b/.test(primary)) return 'IP rules affect who captures value from European know-how and how technology can be transferred, licensed or reused.';
      ...
      if(/semiconductor|microchip|chips act| chip /.test(` ${all} `)) return 'Chip access and production determine whether European industry can use critical hardware without external restrictions.';
      ...
      if(/dual.?use|defen[cs]e|military/.test(primary)) return 'Dual-use policy redirects public funding and adds security, export-control and openness constraints.';
      ...
      if(/data space|interoperab|research infrastructure|infrastructure|federat|data reuse/.test(primary)) return 'Infrastructure and data rules determine whether European firms can access and reuse shared assets across borders.';
      ...
      if(/strategic resilience.*integration bloc|integration blocs.*fragmentation/.test(primary)) return 'Exposure to sanctions and value-chain shocks determines how reliably critical inputs and technologies can be supplied.';
      return 'Europe’s economic security is affected through the paper’s evidence on capacity, access, cost, coordination or external dependence.';
    })();
  ...
  function whyYouShouldCare(x){
    ...
    if(/research security|knowledge security|foreign interference|espionage/.test(n))
      return 'Technology-security rules affect which firms Europe trades with and what know-how can cross borders.';
    ...
    if(/semiconductor|microchip|chips act| chip /.test(` ${n} `))
      return 'Chip access and production determine whether European industry can use critical hardware without external restrictions.';
    ...
    if(/industrial policy|industrial|industry|manufactur|productivity|commerciali|innovation ecosystem|procurement/.test(n))
      return 'Industrial-policy choices affect whether European know-how turns into domestic production, scale and strategic capability.';
    if(/data space|interoperab|research infrastructure|federat|data reuse/.test(n))
      return 'Infrastructure and data rules determine whether European firms can access and reuse shared assets across borders.';
  ...
  function themeWhy(theme){
    ...
    if(/china|de-risk/.test(n)) return 'EU–China economic ties may become narrower or more selective.';
    ...
    if(/economic security/.test(n)) return 'Economic-security policy may change Europe’s dependencies, capacity and room for manoeuvre.';
    if(/competitiveness|capabilit/.test(n)) return "Europe's relative economic and technological strength may shift in areas that shape geopolitical power.";
    ...
    if(/science diplomacy/.test(n)) return 'Economic diplomacy may open, narrow or redirect channels for Europe’s economic cooperation.';
    return '';
  }
  ...
  function signalWhy(x){
    ...
    return "Current geopolitical change may alter Europe's economic security.";
  }
