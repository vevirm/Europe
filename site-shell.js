...
(function(){
  ...
  function build(){
    ...
    overlay.style.cssText='position:fixed;inset:0;z-index:2147483647;background:#0f1f3d;color:#fff;display:flex;align-items:center;justify-content:center;font-family:system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;padding:24px';
    overlay.innerHTML=''
      +'<form style="width:100%;max-width:320px;text-align:center">'
      +'<div style="font-size:13px;letter-spacing:.08em;text-transform:uppercase;opacity:.6;margin-bottom:12px">EU Economic Security Radar</div>'
      +'<h1 style="font-size:20px;font-weight:600;margin:0 0 16px">Open the Radar</h1>'
      +'<input type="password" autocomplete="current-password" aria-label="Password" placeholder="Password" style="width:100%;box-sizing:border-box;padding:12px 14px;font-size:16px;border:1px solid #444;border-radius:8px;background:#0f1f3d;color:#fff;margin-bottom:10px">'
      ...
      +'<div data-err style="min-height:18px;color:#8fb3e0;font-size:13px;margin-top:10px" aria-live="polite"></div>'
      +'</form>';
...
(()=>{
  ...
  const pages=[
    {match:/\/radar\/?$/,key:'radar',title:'Radar',purpose:'Current evidence about Europe’s economic security in a changing geopolitical environment.'},
    {match:/\/read\/?$/,key:'read',title:'Read At Least This',purpose:'A compact map of the main phenomena and the parts that make them up.'},
    {match:/\/frontier\/quick\/?$/,key:'frontier-quick',title:'Matrix',purpose:'Where Europe’s economic security is strong, weak, exposed or dependent.'},
    ...
    {match:/\/priorities\/?$/,key:'priorities',title:'Risks & Opportunities',purpose:'Possible consequences for Europe’s economic security.'},
    ...
    {match:/\/shocks\/?$/,key:'shocks',title:'External Shocks',purpose:'Possible disruptions to Europe’s economic security.'},
  ...
  sidebar.innerHTML=`<a class="site-sidebar-brand" href="${prefix}">EU Economic Security Radar</a><button class="site-menu-toggle" type="button" aria-expanded="false" aria-controls="siteMap">Menu</button><nav id="siteMap">${menu.map(([target,label,cls])=>`<a${cls?` class="${cls}"`:''}${activeFor(target)?' aria-current="page"':''} href="${prefix+target}">${label}</a>`).join('')}</nav>`;
