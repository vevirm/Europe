...
function whyFor(x){
  ...
  if(/funding for democracy and governance/.test(n))
    return 'Funding choices shape European capacity on democratic resilience, governance and foreign interference.';
  ...
  if(/semiconductor|microchip|\bchip\b|foundry|lithograph|wafer/.test(n)){
    if(/supply|depend|chokepoint|shortage|export control|restriction/.test(n)) return 'Chip dependencies can constrain European production when external suppliers or governments restrict access.';
  ...
  if(/artificial intelligence|\bai\b|foundation model|large language model/.test(n)){
    ...
    if(/regulat|govern|standard|liability|rule/.test(n)) return 'Artificial-intelligence rules shape which models, data and deployment practices European firms can use.';
  ...
  if(/quantum/.test(n))
    return 'Quantum capability determines whether European firms can access frontier facilities, expertise and strategic technology without external dependence.';
  if(/critical raw material|critical mineral|rare earth|lithium|cobalt|gallium|germanium|battery/.test(n))
    return 'Strategic-material access determines whether European manufacturing can scale without disruption from concentrated external suppliers.';
  ...
  if(/horizon europe|framework programme|research funding|grant|funding programme|european research council|european innovation council/.test(n))
    return 'Funding choices determine which European capabilities, infrastructure and international partnerships can be sustained.';
  ...
  if(/infrastructure|data space|interoperab|facility|testbed|pilot line/.test(n))
    return 'Shared infrastructure determines which European firms can access scarce facilities, data and specialised technical capability.';
  ...
  if(/spin-?off|entrepreneur|regional innovation|innovation system|innovation ecosystem|absorptive capacity|growth model/.test(n))
    return 'Conversion conditions affect whether European know-how becomes firms, regional capability, productivity and durable economic capacity.';
  ...
  if(/earth observation|satellite|space research/.test(n))
    return 'Shared Earth-observation capability can reduce fragmented access to data and strengthen European infrastructure.';
  if(/industrial|manufactur|commerciali|scale.?up|startup|venture capital|procurement/.test(n))
    return 'Commercialisation conditions determine whether European know-how becomes domestic production, growing firms and strategic capability.';
...
function matrixPair(x,opt={}){
  ...
  const y=limit(opt.why||x?.why||whyFor(x)||'It changes European control or capability in this part of the European economy.',15);
