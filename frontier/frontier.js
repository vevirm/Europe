(function(root,factory){
  ...
  const ROWS=[
    {id:'knowledge',name:'Technology & know-how',short:'Technology',description:'Critical technologies, know-how and their control: export controls, leakage, technology partnerships.'},
    {id:'infrastructure',name:'Energy, materials & infrastructure',short:'Materials & infrastructure',description:'Critical raw materials, energy supply, and critical infrastructure such as ports, grids, cables and networks.'},
    {id:'conversion',name:'Industry, trade & investment',short:'Industry & trade',description:'Industrial capacity, supply chains, market access and foreign investment in and out of Europe.'},
    {id:'rules',name:'Tools, coercion & partners',short:'Tools & partners',description:'The economic-security toolbox, coercion and sanctions, trade rules, and agreements with partners.'}
  ];
  const COLUMNS=[
    {id:'A',name:'More secure, stronger',direction:'Europe gains control and its economy gains',tone:'opportunity'},
    {id:'B',name:'More secure, less open',direction:'Europe gains protection but pays in openness, cost or efficiency',tone:'tradeoff'},
    {id:'C',name:'Open gains, still reliant',direction:'openness or partners bring gains while Europe stays dependent',tone:'exposure'},
    {id:'D',name:'Exposed and weaker',direction:'dependence or coercion costs Europe control and economic strength',tone:'alarm'}
  ];
  const CELL_NAMES={
    knowledge:{A:['Build and secure technology','Europe builds critical-technology capacity it controls'],B:['Controls cost openness','export controls or technology-security rules protect Europe but cost sales, partners or innovation'],C:['Partner technology, still reliant','technology from partners strengthens Europe while it depends on their licences or supply'],D:['Know-how leaks or is cut off','Europe loses technology through leakage, takeovers or foreign export bans']},
    infrastructure:{A:['Secure supplies and networks','Europe diversifies, stockpiles or builds capacity in materials, energy or infrastructure'],B:['Security raises costs','removing risky suppliers or vendors protects Europe but raises cost or causes delay'],C:['Imports work, dependence stays','imports keep Europe supplied while concentration on a few suppliers persists'],D:['Disruption or chokepoint hits','export bans, sabotage or supply cuts hurt European industry and households']},
    conversion:{A:['Industry grows securely','strategic industrial capacity and investment grow in Europe under European control'],B:['Protection at a price','tariffs, trade defence, screening or preference rules protect industry but raise prices or invite retaliation'],C:['Foreign capital, open markets','foreign investment or open markets bring growth while ownership or dependence sits abroad'],D:['Capacity and value move away','overcapacity, subsidies abroad or takeovers erode European industry and jobs']},
    rules:{A:['Tools work, partners align','the toolbox deters coercion and partners coordinate with Europe'],B:['Protection strains openness','defensive tools strain WTO rules, partners or Europe\'s open-economy model'],C:['Openness on others\' terms','agreements and open trade help Europe while key rules are set by others'],D:['Coerced or divided','coercion, secondary sanctions or member-state division leave Europe exposed']}
  };

  const ROW_TERMS={
    knowledge:['technology','technologies','critical technology','critical technologies','semiconductor','semiconductors','chip','chips','artificial intelligence','quantum','biotech','biotechnology','know-how','technology leakage','technology transfer','forced technology transfer','intellectual property','export control','export controls','dual-use','dual use','entity list','licence','license','licensing','outbound investment','technology security','research security','patent','patents'],
    infrastructure:['critical raw material','critical raw materials','critical mineral','critical minerals','rare earth','rare earths','gallium','germanium','graphite','lithium','cobalt','magnet','magnets','raw materials','refining','processing','energy','gas','lng','oil','electricity','grid','energy security','critical infrastructure','port','ports','cable','cables','subsea','undersea','telecom','5g','network','networks','pipeline','shipping','stockpile','stockpiling','infrastructure'],
    conversion:['industry','industrial','manufacturing','manufacturer','factory','factories','plant','production','capacity','supply chain','supply chains','value chain','firm','firms','company','companies','investment','investor','fdi','foreign direct investment','acquisition','takeover','ownership','market','market share','exports','imports','overcapacity','steel','batteries','electric vehicles','solar','competitiveness','jobs'],
    rules:['economic security','toolbox','anti-coercion','coercion','sanction','sanctions','tariff','tariffs','anti-dumping','trade defence','foreign subsidies','screening','regulation','regulatory','rule','rules','wto','agreement','agreements','trade agreement','partnership','partners','like-minded','g7','retaliation','countermeasure','countermeasures','member states','unanimity','coordination','diplomacy','european preference','procurement']
  };
  ...
  function evidenceCandidates(data){
    ...
    for(const e of evidence){
      ...
      const strategicKnowledge=/economic security|economic coercion|weaponi[sz]ed interdependence|strategic dependenc|export control|investment screening|critical raw material|de-?risk/i.test(fullText);
  ...
  function rowScores(x,evidence){
    ...
    if(/\b(critical raw material|critical mineral|rare earth|gallium|germanium|lng|gas supply|energy security|port|subsea cable|undersea cable|grid|5g)\b/.test(all) && /access|supplier|vendor|shortage|capacity|infrastructure|supply|export|ban|diversif|stockpil/.test(all)) scores.infrastructure+=4.5;
    if(/\b(manufactur|production capacity|factory|plant|overcapacity|takeover|acquisition|fdi|foreign investment|relocat|market share|industrial)\b/.test(all)) scores.conversion+=4.5;
    if(/\b(anti[- ]coercion|economic coercion|sanctions?|tariffs?|trade defen[cs]e|anti[- ]dumping|wto|trade agreement|partnership|retaliat|screening regulation|toolbox)\b/.test(all)) scores.rules+=4.5;
    if(/\b(technology leakage|forced technology transfer|critical technolog|semiconductor|chips?|quantum|artificial intelligence|dual[- ]use|entity list|outbound investment|know-how)\b/.test(all)) scores.knowledge+=4.5;
